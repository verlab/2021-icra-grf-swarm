...
{ // constructor
    ...
    for (int i = 0; i < this->robots; i++){
        ...
        this->mocapposes.push_back(p);    
    }
    this->r_cmdvel_.push_back(nh_.advertise<geometry_msgs::Twist>("/epuck_robot_0/mobile_base/cmd_vel", 1, true));
    this->r_cmdvel_.push_back(nh_.advertise<geometry_msgs::Twist>("/epuck_robot_1/mobile_base/cmd_vel", 1, true));
    this->r_cmdvel_.push_back(nh_.advertise<geometry_msgs::Twist>("/epuck_robot_2/mobile_base/cmd_vel", 1, true));
    this->r_cmdvel_.push_back(nh_.advertise<geometry_msgs::Twist>("/epuck_robot_3/mobile_base/cmd_vel", 1, true));
    this->r_cmdvel_.push_back(nh_.advertise<geometry_msgs::Twist>("/epuck_robot_4/mobile_base/cmd_vel", 1, true));
    this->r1_pose = nh_.subscribe("/robot_1589/ground_pose", 1, &Controller::r1_pose_cb, this);
    this->r2_pose = nh_.subscribe("/robot_1555/ground_pose", 1, &Controller::r2_pose_cb, this);
    this->r3_pose = nh_.subscribe("/robot_1593/ground_pose", 1, &Controller::r3_pose_cb, this);
    this->r4_pose = nh_.subscribe("/robot_1570/ground_pose", 1, &Controller::r4_pose_cb, this);
    this->r5_pose = nh_.subscribe("/robot_1554/ground_pose", 1, &Controller::r5_pose_cb, this);
    ...
    // ros::param::get("~worldfile", this->worldfile);
    // this->logginfile = this->worldfile;
    // this->logginfile.replace(this->logginfile.end()-3, this->logginfile.end(), "log");
    ...
    // std::FILE *coords = fopen(this->worldfile.c_str(), "r");
    ...
    for (int i = 0; i < this->robots; ++i){
        ...
        // fscanf(coords, "%f", &vx);
        // fscanf(coords, "%f", &vy);
        r.velocity.x = this->vmax; r.velocity.y = this->vmax;

        // fscanf(coords, "%f", &type);
...
/* Callbacks */
void Controller::r1_pose_cb(const geometry_msgs::Pose2DConstPtr &msg){
    this->mocapposes[0] = *msg;
    ROS_INFO("M0: %f, %f, %f", msg->x, msg->y, msg->theta);
}
void Controller::r2_pose_cb(const geometry_msgs::Pose2DConstPtr &msg){
    this->mocapposes[1] = *msg;
    ROS_INFO("M1: %f, %f, %f", msg->x, msg->y, msg->theta);
}
void Controller::r3_pose_cb(const geometry_msgs::Pose2DConstPtr &msg){
    this->mocapposes[2] = *msg;
    ROS_INFO("M2: %f, %f, %f", msg->x, msg->y, msg->theta);
}
void Controller::r4_pose_cb(const geometry_msgs::Pose2DConstPtr &msg){
    this->mocapposes[3] = *msg;
    ROS_INFO("M3: %f, %f, %f", msg->x, msg->y, msg->theta);
}
void Controller::r5_pose_cb(const geometry_msgs::Pose2DConstPtr &msg){
    this->mocapposes[4] = *msg;
    ROS_INFO("M4: %f, %f, %f", msg->x, msg->y, msg->theta);
}
...
double Controller::fof_Us(Robot r_i, Vector2 v){
    ...
    for (int i = 0; i < obstacles.size(); ++i){
        ...
        // this->mutex.unlock();
        // #endif
        // ROS_INFO("O(%f, %f)", obstacles[i].x, obstacles[i].y);
...
std::vector<Vector2> Controller::getObstaclesPoints(double sensing, Vector2 p){
    ...
    */
    ...
    p1.x = 0; p1.y = 0;
    Vector2 p2;
    p2.x = this->worldsize; p2.y = 0;
    Vector2 p3;
    p3.x = 0; p3.y = this->worldsize;
    Vector2 p4;
...
void Controller::update(long iterations){
    for (int i=0; i < this->robots; i++){
        this->states[i].position.x = this->mocapposes[i].x;
        this->states[i].position.y = this->mocapposes[i].y;
    }
    ...
    /* Command goto (x,y) */
    for (int i = 0; i < this->robots; ++i){
        double theta_ = atan2(this->states[i].position.y- this->mocapposes[i].y, this->states[i].position.x - this->mocapposes[i].x);
        double err = theta_ - this->mocapposes[i].theta;
        ROS_INFO("theta_: %f, theta: %f, err: %f", theta_, this->mocapposes[i].theta, err);
        geometry_msgs::Twist v;
        double velx = sqrt(this->states[i].velocity.y*this->states[i].velocity.y + this->states[i].velocity.x*this->states[i].velocity.x);
        v.linear.x = velx*0.5;
        
        // v.linear.x = 0.4 * (this->states[i].velocity.x * cos(this->mocapposes[i].theta) + this->states[i].velocity.y * sin(this->mocapposes[i].theta));
